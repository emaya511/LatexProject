...
clear;
clc;
ti=["0.01","0.005","0.003","0.001","0.0009","0.0005"];

for i = 1 : numel(ti)
    
    
    name=strcat(ti(i),'\Dynamic');
    %FEM=DYNAMIC("7000\Dynamic");
    FEM=DYNAMIC(name);
    FEM.lg.lgit(sprintf('** Solving for Rho : %s',ti(i)))
    FEM.ReadMesh("meshes\strip.msh");
    name=strcat(ti(i),"\Mat.dat.txt");
    FEM.Mat(1)=MATDat(name);

    
    tx=linspace(0,10,2001);
    ...
    FEM.TS(1)=TimeSeries(tx,x1);
    FEM.TS(2)=TimeSeries(tx,x2);
    FEM.TS(3)=TimeSeries(tx,x3);
    FEM.TS(4)=TimeSeries(tx,x4);
